import { FileText, Plus, Award, Briefcase, Globe, Heart, Info, ChevronDown, Moon, Sun, Trash2, FlaskConical, Sparkles } from 'lucide-react';
...
export default function Header({ onExportPDF }: HeaderProps) {
  ...
  const scoreColor = completionScore < 40 ? 'text-destructive' : completionScore < 70 ? 'text-amber-500' : 'text-emerald-500';
  const scoreLabel = completionScore < 40 ? 'Getting started' : completionScore < 70 ? 'Looking good' : completionScore < 100 ? 'Almost complete' : 'Complete!';
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60" data-testid="header">
      <div className="container flex h-16 items-center justify-between gap-4 px-4 md:px-6">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base font-semibold leading-tight" data-testid="text-title">AI CV Generator</h1>
            <p className="text-xs text-muted-foreground hidden sm:block">Build a professional resume instantly</p>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-3 flex-1 max-w-xs mx-4">
          <div className="flex-1 space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className={cn("text-xs font-semibold", scoreColor)}>{completionScore}% · {scoreLabel}</span>
            </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground">Optional Sections</div>
              <DropdownMenuSeparator />
              {additionalSections.map((section) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && (
                      <span className="ml-auto text-[10px] font-semibold bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">ON</span>
                    )}
                  </DropdownMenuItem>
                );
              })}
          ...
          {onExportPDF && (
            <Button onClick={onExportPDF} size="sm" className="gap-1.5" data-testid="button-export-pdf">
      ...
      <div className="md:hidden px-4 pb-2">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-muted-foreground">CV Completion</span>
          <span className={cn("text-xs font-semibold", scoreColor)}>{completionScore}%</span>
        </div>
        <Progress value={completionScore} className="h-1" />
      </div>
    </header>
  );
}
